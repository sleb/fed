...
async function testMetrics() {
  ...
  try {
    ...
    console.log(
      `- Total Missionaries: ${metrics.overview.totalActiveMissionaries}`,
    );
    console.log(
      `- Total Companionships: ${metrics.overview.totalActiveCompanionships}`,
    );
    ...
    console.log(
      `- Companionships Stats: ${metrics.companionshipStats.length} entries`,
    );
    console.log(
      `- Member Participation: ${metrics.memberParticipation.length} entries`,
    );
    ...
    if (metrics.companionshipStats.length > 0) {
      ...
      metrics.companionshipStats.slice(0, 3).forEach((comp, i) => {
        console.log(
          `  ${i + 1}. ${comp.companionshipArea}: ${comp.mealsThisWeek} meals`,
        );
      });
    }
    ...
    if (metrics.memberParticipation.length > 0) {
      ...
      metrics.memberParticipation.slice(0, 3).forEach((member, i) => {
        console.log(
          `  ${i + 1}. ${member.userName}: ${member.totalSignups} signups`,
        );
      });
    }
